...
stated in this dissertation might have (\formal{https://windtfw.com/agda-symmetries/})
next to it, which is a hyperlink pointing to a HTML rendering of the relevant Agda code. 
The Agda code is also submitted along the dissertation, but it may be difficult to navigate
since as plain source code since without the HTML clickable elements one cannot easily go to
functions and definitions defined in another file within the source code.