...
\[
  \{
    ...
    \}
\]
Well obviously there is only 1 way to sort it, which is given by the order
...
We might ask: how many ways can we sort our finite mulitset of balls?
Well, we know that there are only $3! = 6$ permutations of
...
We define the set $\setord(A)$ to be the set of orderings on $A$,
the set $\setmtol(A)$ to be the set of functions $\term{FiniteMultiset}(A) \to \List(A)$.
There is a function $\otof : \setord(A) \to \setsort(A)$ that maps orderings to
some subset of $\setsort(A) \subset \setmtol(A)$ which we call "sort functions":
...
\begin{center}
    \begin{tikzpicture}
        ...
        % the texts
        \node at (-1.5,0) {$\setord(A)$};
        \node at (1,0) {\tiny$\setsort(A)$};
        \node at (2,0) {\tiny$\setmtol(A)$};
...
This \textit{seems} trivial to prove: we can just construct such a mapping $\otof(R)$ by parameterizing
any sorting algorithms, e.g. insertion sort, by $R$ and we are done!
This however is not enough: we want to show there are exactly 6 sorting functions because
there are exactly 6 orderings. In order words, $\setord(A)$ and $\setsort(A)$ should be
isomorphic, there should exist an inverse mapping to $\otof$ that allows us to construct
an ordering $r$ back given a function $f \in \setsort(A)$, such that
$\forall f \in \setsort(A).\, \otof(\otof^{-1}(f)) = f \land \forall r \in \setord(A).\, \otof^{-1}(\otof(r)) = r$.

\pagebreak
Therefore, to complete this proof formally, we need a plan:
\begin{enumerate}
    \item We need to formalize what exactly $\term{FiniteMultiset}(A)$ and $\List(A)$ are.
    We formalize them in terms of universal algebra: $\term{FiniteMultiset}(A)$ is actually
    the free commutative monoid on $A$ and $\List(A)$ is the free monoid on $A$. We give
    different constructions of them in~\ref{sec:monoids} and ~\ref{sec:commutative-monoids},
    proving these constructions are correct by universal property.
    We also explore the relationship between commutativity and ordering in
    ~\ref{sec:commutative-monoids} and ~\ref{sec:head},
    showing how imposing commutativity on free monoid is same as "forgetting" the order.
    \item We need to exactly nail down what the subset $\Sort(A)$ is, in other words, we need to
    identify what properties functions $f \in \Sort(A) \subset \setmtol(A)$ satisfy which separate
    them from other functions $g \in \setmtol(A)$. We identify two properties in~\ref{sec:sorting},
    giving us two axioms of sorting functions which do not assume a pre-existing total order,
    separating it from other formalizations of correctness of sorting such as 
    \cite{appelVerifiedFunctionalAlgorithms2023}.
    \item Using these properties we have identified, we need to construct a full equivalence
    $\Sort(A) \simeq \Ord(A)$, showing how orders can be mapped to sort functions, which is
    trivial by parameterizing insertion sort with an order, and more interestingly how sort functions
    can be mapped to orders, showing that the properties we have identified for $\Sort(A)$ indeed
    correctly identify our usual notion of sort functions.
\end{enumerate}

We would be constantly working with set quotients and commutativity, which do not behave well
in MLTT since oftentimes they can only be modelled via setoids. While setoids would suffice for
formalizing these structures, they also bring a lot of proof burden which leads to a
phenomenon (un)affectionately named "setoid hell". We have therefore decided to formalize our proofs
for this paper in Cubical Agda, which allows us to work with these structures very naturally with
higher inductive types, and allow for a very simple method to transport proofs between
...
without losing canonicity and computational content of the proof, therefore proofs and functions
transported by univalence would actually compute!
We give a table of our formalization in the appendix \vc{where?}.
