...
\begin{definition}
    ...
    \[
        \begin{aligned}
            ...
            x \preccurlyeq y & \defeq \term{least}(\bag{x, y}) = x \enspace .
        \end{aligned}
    \]
\end{definition}
...
\end{proposition}
\begin{proof}
    For all $x$, $\term{least}(\bag{x, x})$ must be $x$, therefore $x \preccurlyeq x$, giving reflexivity.
    For all $x$ and $y$, given $x \preccurlyeq y$ and $y \preccurlyeq x$,
    we have $\term{least}(\bag{x, y}) = x$ and $\term{least}(\bag{y, x}) = y$.
    Since $\bag{x, y} = \bag{y, x}$, $\term{least}(\bag{x, y}) = \term{least}(\bag{y, x})$,
    therefore we have $x = y$, giving antisymmetry.
    For all $x$ and $y$, $\term{least}(\bag{x, y})$ is merely either $x$ or $y$,
...
\begin{proof}
    ...
    Consider the 3-element $\bag{x,y,z} : \MM(A)$. Let $u$ be $\term{least}(\bag{x,y,z})$,
    by~\cref{sort:head-least} and~\cref{sort:sort-to-order},
    we have $u \preccurlyeq x \land u \preccurlyeq y \land u \preccurlyeq z$.
    Since $u \in \bag{x,y,z}$, $u$ must be one of the element. We prove transitivity by case splitting.
...
$x \preccurlyeq y \defeq \term{least}(\bag{x, y}) = x$,
