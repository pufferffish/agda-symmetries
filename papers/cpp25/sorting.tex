...
\begin{proposition}
    ...
    In a decidable total order, it holds that ${\forall x, y}, {x \leq y} + {y \leq x}$.
    Further, this makes $A$ discrete, that is ${\forall x, y}, {x \id y} + {x \neq y}$.
\end{proposition}
%
...
\begin{definition}[Meet semi-lattice]
    ...
    \begin{itemize}
        \item totality: ${\forall x, y}, {x \meet y \id x} + {x \meet y \id y}$.
    \end{itemize}
\end{definition}
...
\begin{proposition}
    ...
    A decidable total order on $A$ is equivalent to a total meet semi-lattice structure on $A$.
\end{proposition}
\begin{proofsketch}
    Given a (mere) total order $\leq$ on a set $A$,
    we define ${x \meet y} \defeq \term{if} x \leq y \term{then} x \term{else} y$.
    %
    Crucially, this map is \emph{locally-constant}, allowing us to eliminate from an $\hProp$ to a $\hSet$.
    %
    Meets satisfy the universal property of products, that is, ${c \leq a \meet b} \iff {c \leq a} \land {c \leq b}$,
    ...
    which defines a $\hProp$-valued total ordering relation.
    %
...
\begin{definition}[$\term{head}$]
    ...
    Given a total order $\leq$ on a set $A$, we have a commutative monoid structure on $1 + A$,
    ...
    \[
        ...
        \end{aligned}
    \]
    Then, we have a homomorphism \({\term{head} \defeq \ext{\inr}} : {\MM(A) \to 1 + A}\),
...
Given this assumption on $A$, we can sort!
