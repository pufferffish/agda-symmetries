...
Using the correct specification of a permutation relation, we can prove that $\quot{\str{F}(A)}{\approx}$ gives the free
commutative monoid on $A$.

% \begin{lemma}
%     For all $f : A \to X$, $x : \str{F}(A)$, $\ext{f}(q(x))$ reduces to $\exthat{f}(x)$.
% \end{lemma}

\begin{proposition}[Universal property for $\quot{\str{F}(A)}{\approx}$]
